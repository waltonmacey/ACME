...
find $direct -name '*.F90' | xargs grep -l "\s$varIn\>" | cut -d/ -f1-9 |sort -u
...
varDef=$(find $direct -name '*.F90' | xargs sed 's/!.*//' | grep "\b$varIn\b" | grep
-o "\suse\s" |wc -l)

#Return Assignment count
varAs=$(find $direct -name '*.F90' | xargs sed 's/!.*//' | grep "\b$varIn\b" | grep -
o "=>" |wc -l)

#Return Association count
varAsc2=$(find $direct -name '*.F90' | xargs sed 's/!.*//' | grep "\b$varIn\b" | grep
 -o "\s=\s" | wc -l)
varAsc3=$(find $direct -name '*.F90' | xargs sed 's/!.*//' | grep "\b$varIn\b" | grep
 -o "\s==\s" |wc -l)
...
do
  case $opt in
      ...
      "Replace by usage type")
                ...
                echo $strA
                options=("Definition" "Association" "Assignment" "Replace all remaini
ng" "Quit")
                select opt in "${options[@]}"
                do
                        case $opt in
                                "Definition")
                                        ...
                                        echo 'Enter new replacement variable'
                                        read varReplace
                                        find $direct -name '*.F90' | xargs sed -i "/\
suse\s/s/\b$varIn\b/$varReplace/g"
                                ;;
                                "Association")
                                        #Call for input
                                        echo 'Enter new replacement variable'
...
s=\s/s/\b$varIn\b/$varReplace/g"
                                ...
                                "Replace all remaining")
                                        ...
                                        echo 'Enter new replacement variable'
                                        read varReplace
                                        #find $direct -name '*.F90' | xargs sed -i "s
/\b$varIn\b/$varReplace/g"
                                ;;
                ...
                select opt in "${options[@]}"
                        ...
                        case $opt in
                                "Definition")
                                        ...
                                        echo 'Enter new replacement variable'
                                        read varReplace
                                        #find $direct -name '*.F90' | xargs sed -n "/
\suse\s/s/\b$varIn\b/$varReplace/gp" | less
                                        find $direct -name '*.F90' | xargs perl -e 'f
or my $f (@ARGV){ my $f1=$f;$f1=~s|.*'$direct'||;$. = 0; open A, "$f"; while (<A>) {
if ($_=~ m/\suse\s/){if ($_=~m/\b'$varIn'\b/){ print "$f1:$.\n\<$_\>"; $_=~s/\b'$varI
n'\b/'$varReplace'/g;print $_;}}}}' | less
                                ;;
				"Association")
                                        #Call for input
                                        echo 'Enter new replacement variable'
                                        read varReplace
                                        find $direct -name '*.F90' | xargs sed -n "/=
>/s/\b$varIn\b/$varReplace/gp" | less
                                ;;
                                "Assignment")
                                        #Call for input
                                        echo 'Enter new replacement variable'
                                        read varReplace
                                        find $direct -name '*.F90' | xargs sed -n "/\
s==\s/s/\b$varIn\b/$varReplace/gp" | less
                                        find $direct -name '*.F90' | xargs sed -n "/\
s=\s/s/\b$varIn\b/$varReplace/gp" | less
                                ;;
                                "Replace all")
                                        ...
                                        echo 'Enter new replacement variable'
                                        read varReplace
                                        find $direct -name '*.F90' | xargs sed -n "s/
\b$varIn\b/$varReplace/gp" | less
                                ;;
                                "Quit")
                                        break
                                        ;;
