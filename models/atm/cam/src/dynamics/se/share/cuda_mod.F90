...
!Put everything CUDA-specific in here so it doesn't get compiled without -Mcuda enabled on a PGI compiler
  ...
  real (kind=real_kind),device,allocatable,dimension(:,:,:,:,:)   :: tensorvisc_d
...
!$OMP MASTER
    ...
    allocate( tensorvisc_d             (2,2,np,np                    ,nelemd) , stat = ierr ); _CHECK(__LINE__)
    ...
    do ie = 1,nelemd
      ...
      ierr = cudaMemcpy( tensorvisc_d             (1,1,1,1  ,ie) , elem(ie)%tensorvisc              , size(elem(ie)%tensorvisc             ) , cudaMemcpyHostToDevice ); _CHECK(__LINE__)
    enddo
    do k = 1 , nlev    
...
subroutine advance_hypervis_scalar_cuda( edgeAdv , elem , hvcoord , hybrid , deriv , nt , nt_qdp , nets , nete , dt2 )
  ...
  use control_mod    , only : rsplit, nu_q, hypervis_order, hypervis_subcycle_q, nu_p, hypervis_scaling, hypervis_power
  use parallel_mod   , only : iam
  implicit none
...
!$OMP MASTER
    ...
    call hypervis_kernel1<<<griddim,blockdim,0,streams(1)>>>( qdp_d , qtens_d , dp_d , dinv_d , variable_hyperviscosity_d , tensorVisc_d , dpdiss_ave_d , spheremp_d , &
                                                              deriv_dvv_d , hyai_d , hybi_d , hvcoord%ps0 , 1 , nelemd , dt , nt_qdp , nu_p , hypervis_power , hypervis_scaling ); _CHECK(__LINE__)
    ...
    blockdim = dim3( np*np*numk_hyp , 1 , 1 )
    griddim  = dim3( int(ceiling(dble(nlev)/numk_hyp))*qsize_d*nelemd , 1 , 1 )
    call hypervis_kernel2<<<griddim,blockdim,0,streams(1)>>>( qdp_d , qtens_d , dp_d , dinv_d , variable_hyperviscosity_d , tensorVisc_d , spheremp_d , rspheremp_d , deriv_dvv_d , &
                                                              nu_q , 1 , nelemd , dt , nt_qdp , hypervis_power , hypervis_scaling ); _CHECK(__LINE__)
    blockdim = dim3( np*np*numk_lim2d , 1 , 1 )
    griddim  = dim3( int(ceiling(dble(nlev)/numk_lim2d))*qsize_d*nelemd , 1 , 1 )
...
attributes(global) subroutine hypervis_kernel1( Qdp , qtens , dp , dinv , variable_hyperviscosity , tensorVisc , dpdiss_ave , spheremp , deriv_dvv , hyai , hybi , ps0 , nets , nete , dt , nt , nu_p , &
                                                hypervis_power , hypervis_scaling )
  implicit none
  real(kind=real_kind), dimension(np,np,nlev,qsize_d,timelevels,nets:nete), intent(in   ) :: Qdp
  real(kind=real_kind), dimension(np,np,nlev,qsize_d           ,nets:nete), intent(  out) :: Qtens
  ...
  real(kind=real_kind), dimension(2,2,np,np                    ,nets:nete), intent(in   ) :: tensorVisc
  ...
  real(kind=real_kind), value                                             , intent(in   ) :: hypervis_power , hypervis_scaling
  ...
  call syncthreads()
  qtens(i,j,k,q,ie) = laplace_sphere_wk(i,j,ie,kk,s,dinv_s,spheremp_s,variable_hyperviscosity,tensorVisc,deriv_dvv_s,nets,nete,var_coef1,hypervis_power,hypervis_scaling)
end subroutine hypervis_kernel1



attributes(global) subroutine hypervis_kernel2( Qdp , qtens , dp , dinv , variable_hyperviscosity , tensorVisc , spheremp , rspheremp , deriv_dvv , nu_q , nets , nete , dt , nt , hypervis_power , hypervis_scaling )
  implicit none
  real(kind=real_kind), dimension(np,np,nlev,qsize_d,timelevels,nets:nete), intent(inout) :: Qdp
  real(kind=real_kind), dimension(np,np,nlev,qsize_d           ,nets:nete), intent(in   ) :: Qtens
  real(kind=real_kind), dimension(np,np,nlev                   ,nets:nete), intent(in   ) :: dp
  ...
  call syncthreads()
  Qdp(i,j,k,q,nt,ie) = Qdp(i,j,k,q,nt,ie)*spheremp_s(i,j)-dt*nu_q*laplace_sphere_wk(i,j,ie,kk,s,dinv_s,spheremp_s,variable_hyperviscosity,tensorVisc,deriv_dvv_s,nets,nete,.true.,hypervis_power,hypervis_scaling)
end subroutine hypervis_kernel2



attributes(device) function laplace_sphere_wk(i,j,ie,k,s,dinv,spheremp,variable_hyperviscosity,tensorVisc,deriv_dvv,nets,nete,var_coef,hypervis_power,hypervis_scaling) result(lapl)
  implicit none
  ...
  ds = gradient_sphere(i,j,ie,k,s,dinv,variable_hyperviscosity,tensorVisc,deriv_dvv,nets,nete,var_coef,hypervis_power,hypervis_scaling)
  lapl = divergence_sphere_wk(i,j,ie,k,ds,s,dinv,spheremp,deriv_dvv,nets,nete)
end function laplace_sphere_wk



attributes(device) function gradient_sphere(i,j,ie,k,s,dinv,variable_hyperviscosity,tensorVisc,deriv_dvv,nets,nete,var_coef,hypervis_power,hypervis_scaling) result(ds)
  implicit none
  integer,                                              intent(in) :: nets, nete, i, j, ie, k
  ...
  enddo
  ds(1) = ( dinv((j-1)*np+i,1,1)*dsdx00 + dinv((j-1)*np+i,2,1)*dsdy00 ) * rrearth_d
  ds(2) = ( dinv((j-1)*np+i,1,2)*dsdx00 + dinv((j-1)*np+i,2,2)*dsdy00 ) * rrearth_d
  if (var_coef) then
    if (hypervis_power /= 0 ) then
      ! scalar viscosity with variable coefficient
      ds(1) = ds(1) * variable_hyperviscosity(i,j,ie)
      ds(2) = ds(2) * variable_hyperviscosity(i,j,ie)
    else if (hypervis_scaling /=0 ) then
      ! tensor hv, (3)
      ds(1) = sum(ds(:)*tensorVisc(1,:,i,j,ie))
      ds(2) = sum(ds(:)*tensorVisc(2,:,i,j,ie))
    endif
  endif
...
attributes(global) subroutine compute_initial_data_for_limiter_full (Qtens, dpdiss_biharmonic, qtens_biharmonic, dp, spheremp, dp_star, divdp, rhs_viss, nu_p, nu_q, dt, nets , nete )
 ...
 implicit none
  ...
  real(kind=real_kind), dimension(np,np                        ,nets:nete), intent(in   ) :: spheremp
  real(kind=real_kind), dimension(np,np,nlev                   ,nets:nete), intent(in   ) :: dp
  real(kind=real_kind), dimension(np,np,nlev                   ,nets:nete), intent(  out) :: dp_star
...
!    if ( k <= nlev .and. q <= qsize_d .and. ie <= nete ) then
      ...
      if ( nu_p > 0 .and. rhs_viss /= 0 ) then
         dp_star(i,j,k,ie) = dp_star(i,j,k,ie) - rhs_viss * dt * nu_q * dpdiss_biharmonic(i,j,k,ie) / spheremp(i,j,ie)
      endif
...
!    k  = modulo(kloop-1,ks)*numk_lim8 + kk
    if ( k <= nlev .and. q <= qsize_d .and. ie <= nete ) then
      x_s(ij,kk)=Qtens(i,j,k,q,ie)/dp_star(i,j,k,ie)
...
!    k  = modulo(kloop-1,ks)*numk_lim8 + kk
 ...
 call syncthreads()
 Qtens(i,j,k,q,ie)=x_s(ij,kk)*dp_star(i,j,k,ie)
 call syncthreads()
    ...
    do ie = 1 , nelemd
      do kc = 1 , vlyr/kchunk+1
        ...
        do kk = 1 , kchunk
          ...
          if (k <= vlyr) then
            do i = 1 , np
              vtmp(i +(1 -1)*np,kk) = min( vtmp(i +(1 -1)*np,kk) , edge_buf(kptr+k,getmapP(south,ie)+i) )
              vtmp(np+(i -1)*np,kk) = min( vtmp(np+(i -1)*np,kk) , edge_buf(kptr+k,getmapP(east ,ie)+i) )
              vtmp(i +(np-1)*np,kk) = min( vtmp(i +(np-1)*np,kk) , edge_buf(kptr+k,getmapP(north,ie)+i) )
              vtmp(1 +(i -1)*np,kk) = min( vtmp(1 +(i -1)*np,kk) , edge_buf(kptr+k,getmapP(west ,ie)+i) )
            enddo
            do i = 0 , max_corner_elem-1
              ll = swest+0*max_corner_elem+i; if(getmapP(ll,ie) /= -1) vtmp(1 +(1 -1)*np,kk) = min( vtmp(1 +(1 -1)*np,kk) , edge_buf(kptr+k,getmapP(ll,ie)+1) )
              ll = swest+1*max_corner_elem+i; if(getmapP(ll,ie) /= -1) vtmp(np+(1 -1)*np,kk) = min( vtmp(np+(1 -1)*np,kk) , edge_buf(kptr+k,getmapP(ll,ie)+1) )
              ll = swest+2*max_corner_elem+i; if(getmapP(ll,ie) /= -1) vtmp(1 +(np-1)*np,kk) = min( vtmp(1 +(np-1)*np,kk) , edge_buf(kptr+k,getmapP(ll,ie)+1) )
              ll = swest+3*max_corner_elem+i; if(getmapP(ll,ie) /= -1) vtmp(np+(np-1)*np,kk) = min( vtmp(np+(np-1)*np,kk) , edge_buf(kptr+k,getmapP(ll,ie)+1) )
            enddo
          endif
        enddo
        !$acc loop vector
    ...
    do ie = 1 , nelemd
      do kc = 1 , vlyr/kchunk+1
        ...
        do kk = 1 , kchunk
          ...
          if (k <= vlyr) then
            do i = 1 , np
              vtmp(i +(1 -1)*np,kk) = max( vtmp(i +(1 -1)*np,kk) , edge_buf(kptr+k,getmapP(south,ie)+i) )
              vtmp(np+(i -1)*np,kk) = max( vtmp(np+(i -1)*np,kk) , edge_buf(kptr+k,getmapP(east ,ie)+i) )
              vtmp(i +(np-1)*np,kk) = max( vtmp(i +(np-1)*np,kk) , edge_buf(kptr+k,getmapP(north,ie)+i) )
              vtmp(1 +(i -1)*np,kk) = max( vtmp(1 +(i -1)*np,kk) , edge_buf(kptr+k,getmapP(west ,ie)+i) )
            enddo
            do i = 0 , max_corner_elem-1
              ll = swest+0*max_corner_elem+i; if(getmapP(ll,ie) /= -1) vtmp(1 +(1 -1)*np,kk) = max( vtmp(1 +(1 -1)*np,kk) , edge_buf(kptr+k,getmapP(ll,ie)+1) )
              ll = swest+1*max_corner_elem+i; if(getmapP(ll,ie) /= -1) vtmp(np+(1 -1)*np,kk) = max( vtmp(np+(1 -1)*np,kk) , edge_buf(kptr+k,getmapP(ll,ie)+1) )
              ll = swest+2*max_corner_elem+i; if(getmapP(ll,ie) /= -1) vtmp(1 +(np-1)*np,kk) = max( vtmp(1 +(np-1)*np,kk) , edge_buf(kptr+k,getmapP(ll,ie)+1) )
              ll = swest+3*max_corner_elem+i; if(getmapP(ll,ie) /= -1) vtmp(np+(np-1)*np,kk) = max( vtmp(np+(np-1)*np,kk) , edge_buf(kptr+k,getmapP(ll,ie)+1) )
            enddo
          endif
        enddo
        !$acc loop vector
...
!     note: for this form of the operarad(s) does not need to be made C0
    ...
    !$acc routine(gradient_sphere_loc) vector
    !$acc routine(divergence_sphere_wk_loc) vector
    !$acc parallel loop gang collapse(3) private(lap_p,grads,vtemp) present(dinv,tensorvisc,spheremp,hypervis_power,hypervis_scaling,variable_hyperviscosity,deriv_dvv) deviceptr(s,laplace)
    do ie = 1 , nelemd
      do q = 1 , qsize
        do ks = 1 , nlev/kchunk+1
