...
! Register fields in the constituent array and the physics buffer.
   ...
   call pbuf_add_field('cu_cmfum',      'global', dtype_r8, (/pcols,pver,dyn_time_lvls/),       cu_cmfum_idx)
   ...
   call pbuf_add_field('cu_cmfrd',      'global', dtype_r8, (/pcols,pver,dyn_time_lvls/),       cu_cmfrd_idx)
   call pbuf_add_field('cu_thlrd',      'global', dtype_r8, (/pcols,pver,dyn_time_lvls/),       cu_thlrd_idx)
   call pbuf_add_field('cu_qtrd',       'global', dtype_r8, (/pcols,pver,dyn_time_lvls/),       cu_qtrd_idx)
   call pbuf_add_field('cu_urd',        'global', dtype_r8, (/pcols,pver,dyn_time_lvls/),       cu_urd_idx)
   call pbuf_add_field('cu_vrd',        'global', dtype_r8, (/pcols,pver,dyn_time_lvls/),       cu_vrd_idx)
...
subroutine unicon_cam_init(pbuf2d)
   ...
   ipbl_idx    = pbuf_get_index('ipbl')
   kpblh_idx    = pbuf_get_index('kpblh')
   pblh_idx    = pbuf_get_index('pblh')
   wstarPBL_idx    = pbuf_get_index('wstarPBL')
   tkes_idx    = pbuf_get_index('tkes')
   ...
   if (is_first_step()) then
      ...
      if (cu_cmfum_idx > 0) then
         call pbuf_set_field(pbuf2d, cu_cmfum_idx,      0.0_r8)
      else
         call endrun(sub//'cu_cmfum not in pbuf')
      end if
      ...
      if (cu_cmfrd_idx > 0) then
         call pbuf_set_field(pbuf2d, cu_cmfrd_idx,      0.0_r8)
      else
         call endrun(sub//'cu_cmfrd not in pbuf')
      end if
      if (cu_thlrd_idx > 0) then
         call pbuf_set_field(pbuf2d, cu_thlrd_idx,      0.0_r8)
      else
         call endrun(sub//'cu_thlrd not in pbuf')
      end if
      if (cu_qtrd_idx > 0) then
         call pbuf_set_field(pbuf2d, cu_qtrd_idx,       0.0_r8)
      else
         call endrun(sub//'cu_qtrd not in pbuf')
      end if
      if (cu_urd_idx > 0) then
         call pbuf_set_field(pbuf2d, cu_urd_idx,        0.0_r8)
      else
         call endrun(sub//'cu_urd not in pbuf')
      end if
      if (cu_vrd_idx > 0) then
         call pbuf_set_field(pbuf2d, cu_vrd_idx,        0.0_r8)
      else
         call endrun(sub//'cu_vrd not in pbuf')
      end if
      if (cu_qlrd_idx > 0) then
         call pbuf_set_field(pbuf2d, cu_qlrd_idx,       0.0_r8)
      else
         call endrun(sub//'cu_qlrd not in pbuf')
      end if
      if (cu_qird_idx > 0) then
         call pbuf_set_field(pbuf2d, cu_qird_idx,       0.0_r8)
      else
         call endrun(sub//'cu_qird not in pbuf')
      end if
      if (cu_trrd_idx > 0) then
         call pbuf_set_field(pbuf2d, cu_trrd_idx,       0.0_r8)
      else
...
subroutine unicon_cam_tend(dt, state, cam_in, sgh, sgh30, &
                           pbuf, ptend, out)
   ...
   real(r8),                  intent(in)  :: sgh(mix)   ! Std dev of subgrid topographic height [ meter ] 
   ...
   integer(i4), pointer, dimension(:) :: & ! (mix)
      ipbl,          &! If 1(0), PBL is CL(STL)
      kpblh_inv       ! Layer index with PBL top in it or at the base interface

   real(r8), pointer, dimension(:) :: & ! (mix)
      pblh,          &! PBL top height [m]
      wstar,         &! Turbulent convective velocity scale within PBL [ m / s ]
      tkes,          &! Turbulent kinetic energy at surface directly from UW PBL scheme [ m / s ]
   ...
   real(r8) :: u0_c(mix,mkx)             !  Environmental zonal wind
   real(r8) :: v0_c(mix,mkx)             !  Environmental meridional wind
   ...
   call pbuf_get_field(pbuf, wstarPBL_idx,    wstar)
   ...
   call pbuf_get_field(pbuf, cu_cmfum_idx, cu_cmfum, start=(/1,1,itim/),   kount=(/pcols,pver,1/))
   ...
   call pbuf_get_field(pbuf, cu_cmfrd_idx, cu_cmfrd, start=(/1,1,itim/),   kount=(/pcols,pver,1/))
   call pbuf_get_field(pbuf, cu_thlrd_idx, cu_thlrd, start=(/1,1,itim/),   kount=(/pcols,pver,1/))
   call pbuf_get_field(pbuf, cu_qtrd_idx,  cu_qtrd,  start=(/1,1,itim/),   kount=(/pcols,pver,1/))
   call pbuf_get_field(pbuf, cu_urd_idx,   cu_urd,   start=(/1,1,itim/),   kount=(/pcols,pver,1/))
   call pbuf_get_field(pbuf, cu_vrd_idx,   cu_vrd,   start=(/1,1,itim/),   kount=(/pcols,pver,1/))
   call pbuf_get_field(pbuf, cu_qlrd_idx,  cu_qlrd,  start=(/1,1,itim/),   kount=(/pcols,pver,1/))
   ...
   call compute_unicon( mix       , mkx        , iend      , ncnst    , mit     , dt      , &
                        ...
                        cam_in%ocnfrac, cam_in%landfrac, cam_in%icefrac, sgh    , sgh30   , &
   ...
   u0_c(:iend,:mkx)   =  u0(:iend,:mkx) +  uten(:iend,:mkx)*dt
   v0_c(:iend,:mkx)   =  v0(:iend,:mkx) +  vten(:iend,:mkx)*dt
