  ...
  !--------------------------------------------------- !
   ...
   use physconst,        only: cpair, latvap, latice, rh2o, gravit
   ...
   integer,  parameter          :: i_rhminl     =  0            ! This is for liquid stratus fraction.
                                                                ...
                                                                !        In this case, 'tau_detw' [s] and 'c_aniso' [no unit] should be specified below. 

   integer,  parameter          :: i_rhmini     =  0            ! This is for ice stratus fraction.
                                                                ! If 0 : Original fixed critical RH from the namelist.
                                                                ! If 1 : Add convective detrainment effect on the above '0' option. 
                                                                !        In this case, 'tau_deti' [s] should be specified below.
                                                                ! If 2 : Use fully scale-adaptive method.
                                                                !        In this case, 'tau_deti' [s] and 'c_aniso' [no unit] should be specified below. 
                                                                ! Note that 'micro_mg_cam' is using below 'rhmini_org', regardless of 'i_rhmini'.
                                                                ! This connection should be built in future.

   real(r8), parameter          :: tau_detw     = 20._r8        ! Dissipation time scale of convective liquid condensate detrained into the clear portion. [hr]. 0.5-3 hr is possible.
   real(r8), parameter          :: tau_deti     =  1._r8        ! Dissipation time scale of convective ice    condensate detrained into the clear portion. [hr]. 0.5-3 hr is possible.
   ...
   ! -------------------------- !
   real(r8), public,  parameter :: rhmini_org   = 0.95_r8       ! Minimum rh for ice cloud fraction > 0.
   ...
   real(r8), private            :: rhminl_org                   ! Critical RH for low-level  liquid stratus clouds
   real(r8), private            :: rhminl_adj_land_org          ! rhminl adjustment for snowfree land
   real(r8), private            :: rhminh_org                   ! Critical RH for high-level liquid stratus clouds
   ...
   call cldfrc_getparams(rhminl_out = rhminl_org, rhminl_adj_land_out = rhminl_adj_land_org,  &
                         rhminh_out = rhminh_org, premit_out = premit,   premib_out = premib, &
                         iceopt_out = iceopt, icecrit_out = icecrit)

   if( masterproc ) then
       write(iulog,*) 'ini_macro: tuning parameters : rhminl = ', rhminl_org, &
                                                     'rhminl_adj_land = ', rhminl_adj_land_org, & 
                                                     'rhminh = ', rhminh_org, & 
                                                     'premit = ', premit, & 
                                                     'premib = ',  premib,  &
                                                     'iceopt = ',  iceopt,  &
 ...
 ! Sungsu added below for diagnostic output

   call addfld ('RHMIN_LIQ',     'fraction', pver, 'A', 'Default critical RH for liquid-stratus'                       &
, phys_decomp)
   call addfld ('RHMIN_ICE',     'fraction', pver, 'A', 'Default critical RH for    ice-stratus'                       &
, phys_decomp)
   call addfld ('DRHMINPBL_LIQ', 'fraction', pver, 'A', 'Drop of liquid-stratus critical RH by PBL turbulence'         &
, phys_decomp)
   call addfld ('DRHMINPBL_ICE', 'fraction', pver, 'A', 'Drop of    ice-stratus critical RH by PBL turbulence'         &
, phys_decomp)
   call addfld ('DRHMINDET_LIQ', 'fraction', pver, 'A', 'Drop of liquid-stratus critical RH by convective detrainment' &
, phys_decomp)
   call addfld ('DRHMINDET_ICE', 'fraction', pver, 'A', 'Drop of    ice-stratus critical RH by convective detrainment' &
, phys_decomp)

   call addfld ('CMFR_DET',      'kg/m2/s',  pver, 'A', 'Detrained convective mass flux'                               &
, phys_decomp)
   call addfld ('QLR_DET',       'kg/kg',    pver, 'A', 'Detrained convective LWC'                                     &
, phys_decomp)
   call addfld ('QIR_DET',       'kg/kg',    pver, 'A', 'Detrained convective IWC'                                     &
, phys_decomp)
   call addfld ('CLR_LIQ',       'fraction', pver, 'A', 'Clear sky fraction for liquid stratus'                        &
, phys_decomp)
   call addfld ('CLR_ICE',       'fraction', pver, 'A', 'Clear sky fraction for ice stratus'                           &
, phys_decomp)

   call addfld ('A_DET',         'fraction', pver, 'A', 'Cloud fraction generated by detrained convective condensate'  &
, phys_decomp)

 ! Sungsu added above for diagnostic output
 ...
 ! Sungsu
   ...
   real(r8), intent(in)    :: tke(pcols,pverp)             ! TKE from the PBL scheme
   real(r8), intent(in)    :: qtl_flx(pcols,pverp)         ! overbar(w'qtl') from PBL scheme where qtl = qv + ql
   real(r8), intent(in)    :: qti_flx(pcols,pverp)         ! overbar(w'qti') from PBL scheme where qti = qv + qi
   real(r8), intent(in)    :: cmfr_det(pcols,pver)         ! Detrained mass flux from the convection scheme
   real(r8), intent(in)    :: qlr_det(pcols,pver)          ! Detrained        ql from the convection scheme
   real(r8), intent(in)    :: qir_det(pcols,pver)          ! Detrained        qi from the convection scheme
 ! Sungsu
 ...
 ! Coefficient for computing QQ and related processes
   ...
   real(r8) esat_a(pcols,pver)                             ! Saturation water vapor pressure [Pa]
   real(r8) qsat_a(pcols,pver)                             ! Saturation water vapor specific humidity [kg/kg]
   real(r8) dqsdT_a(pcols,pver)                            ! dqsat/dT [kg/kg/K]
 ...
 ! Sungsu
   ...
   real(r8) esatw_tmp(pcols,pver)                          ! Saturation water vapor pressure over water [Pa]
   real(r8) qsatw_tmp(pcols,pver)                          ! Saturation water vapor specific humidity over water [kg/kg]
   real(r8) esati_tmp(pcols,pver)                          ! Saturation water vapor pressure over ice [Pa]
   real(r8) qsati_tmp(pcols,pver)                          ! Saturation water vapor specific humidity over ice [kg/kg]
   real(r8) sig_qtl(pcols,pver)       
   real(r8) sig_qti(pcols,pver)        
 ...
 ! Other
   ...
   ! Sungsu
     esatw_tmp(:ncol,:)       = 0._r8     
     qsatw_tmp(:ncol,:)       = 0._r8    
     esati_tmp(:ncol,:)       = 0._r8     
     qsati_tmp(:ncol,:)       = 0._r8    
     sig_qtl(:ncol,:)         = 0._r8
     sig_qti(:ncol,:)         = 0._r8
     d_rhmin_liq_PBL(:ncol,:) = 0._r8
     d_rhmin_ice_PBL(:ncol,:) = 0._r8
     d_rhmin_liq_det(:ncol,:) = 0._r8
     d_rhmin_ice_det(:ncol,:) = 0._r8
     rhmini_arr(:ncol,:)          = 0._r8
     rhminl_arr(:ncol,:)          = 0._r8
     rhminl_adj_land_arr(:ncol,:) = 0._r8
     rhminh_arr(:ncol,:)          = 0._r8
     rhmin_liq_diag(:ncol,:)      = 0._r8
     rhmin_ice_diag(:ncol,:)      = 0._r8
   ! Sungsu
   ...
   do k = top_lev, pver
      call qsat_water(T0(1:ncol,k), p(1:ncol,k), esatw_tmp(1:ncol,k), qsatw_tmp(1:ncol,k))
      call   qsat_ice(T0(1:ncol,k), p(1:ncol,k), esati_tmp(1:ncol,k), qsati_tmp(1:ncol,k))
      do i = 1, ncol
         ! ---------------------------------------------------------------------------- !
         ! Compute the drop of critical RH by the variability induced by PBL turbulence !
         ! ---------------------------------------------------------------------------- !
         sig_qtl(i,k) = 0.5_r8 * ( qtl_flx(i,k)   / sqrt(max(qsmall,tke(i,k))) + & 
                                   qtl_flx(i,k+1) / sqrt(max(qsmall,tke(i,k+1))) )
         sig_qti(i,k) = 0.5_r8 * ( qti_flx(i,k)   / sqrt(max(qsmall,tke(i,k))) + & 
                                   qti_flx(i,k+1) / sqrt(max(qsmall,tke(i,k+1))) )
         d_rhmin_liq_PBL(i,k) = c_aniso*sig_qtl(i,k)/max(qsmall,qsatw_tmp(i,k)) 
         d_rhmin_liq_PBL(i,k) = max(0._r8,min(0.5_r8,d_rhmin_liq_PBL(i,k)))
         d_rhmin_ice_PBL(i,k) = c_aniso*sig_qti(i,k)/max(qsmall,qsati_tmp(i,k)) 
         ...
         ! ----------------------------------------------------------------------------- !
       ! d_rhmin_liq_det(i,k) = tau_detw*(gravit/dp(i,k))*cmfr_det(i,k)*clrw_old(i,k)* & 
       !                                 (qlr_det(i,k)/max(qsmall,qsatw_tmp(i,k))) 
       ! d_rhmin_liq_det(i,k) = max(0._r8,min(0.5_r8,d_rhmin_liq_det(i,k)))
       ! d_rhmin_ice_det(i,k) = tau_deti*(gravit/dp(i,k))*cmfr_det(i,k)*clri_old(i,k)* & 
       !                                 (qir_det(i,k)/max(qsmall,qsati_tmp(i,k))) 
       ! d_rhmin_ice_det(i,k) = max(0._r8,min(0.5_r8,d_rhmin_ice_det(i,k)))
         ...
         d_rhmin_ice_det(i,k) = max(0._r8,min(0.5_r8,d_rhmin_ice_det(i,k)))
         ! ---------------------------------- !
         ! Choose critical RH for ice stratus !
         ! ---------------------------------- !
         if( i_rhmini .eq. 0 ) then
             rhmini_arr(i,k) = rhmini_org
         elseif( i_rhmini .eq. 1 ) then ! Original + Convective detrainment
             rhmini_arr(i,k) = rhmini_org - d_rhmin_ice_det(i,k)
         elseif( i_rhmini .eq. 2 ) then ! PBL turbulence + Convective detrainment
         ...
         ! ------------------------------------- !
         if( i_rhminl .eq. 0 ) then
             rhminl_arr(i,k) = rhminl_org
             rhminl_adj_land_arr(i,k) = rhminl_adj_land_org
             rhminh_arr(i,k) = rhminh_org
         elseif( i_rhminl .eq. 1 ) then ! Original + Convective detrainment
             rhminl_arr(i,k) = rhminl_org - d_rhmin_liq_det(i,k)
             rhminl_adj_land_arr(i,k) = rhminl_adj_land_org
             rhminh_arr(i,k) = rhminh_org - d_rhmin_liq_det(i,k)
         elseif( i_rhminl .eq. 2 ) then ! PBL turbulence + Convective detrainment
         ...
         ! -------------------------------------------------------------------------------- !
         ! Diagnostic output : Compute default critical RH as a function of height and      !
         !                     surface type as in the current code.                         !
         ! -------------------------------------------------------------------------------- !
         land = nint(landfrac(i)) == 1
         rhmin_ice_diag(i,k) = rhminh_org
         if( p(i,k) .ge. premib ) then
             if( land .and. (snowh(i).le.0.000001_r8) ) then
                 rhmin_liq_diag(i,k) = rhminl_org - rhminl_adj_land_org
             else
                 rhmin_liq_diag(i,k) = rhminl_org
             endif
         elseif( p(i,k) .lt. premit ) then
             rhmin_liq_diag(i,k) = rhminh_org
         else
             tmp = (premib-(max(p(i,k),premit)))/(premib-premit)
             rhmin_liq_diag(i,k) = rhminh_org*tmp + rhminl_org*(1.0_r8-tmp)
         endif         
      enddo
   enddo
   ...
   do k = top_lev, pver
      ...
      call qsat_water(T_05(1:ncol,k), p(1:ncol,k), &
                      esat_a(1:ncol,k), qsat_a(1:ncol,k), dqsdt=dqsdT_a(1:ncol,k))
   enddo
...
!           esat_a(1:ncol), qsat_a(1:ncol), dqsdt=dqsdT_a(1:ncol))
      call qsat_water(T(1:ncol,k), p(1:ncol,k), &
                      esat_b(1:ncol,k), qsat_b(1:ncol,k), dqsdt=dqsdT_b(1:ncol,k))
      ...
      if( CAMstfrac ) then
          call astG_RHU(U_nc(:,k),p(:,k),qv(:,k),landfrac(:),snowh(:),al_st_nc(:,k),G_nc(:,k),ncol,&
                        rhminl_in=rhminl_arr(:,k),rhminl_adj_land_in=rhminl_adj_land_arr(:,k),&
                        rhminh_in=rhminh_arr(:,k))                          
      else
      ...
      call aist_vector(qv(:,k),T(:,k),p(:,k),qi(:,k),landfrac(:),snowh(:),ai_st_nc(:,k),ncol,&
                       rhmini_in=rhmini_arr(:,k),rhminl_in=rhminl_arr(:,k),&
                       rhminl_adj_land_in=rhminl_adj_land_arr(:,k),rhminh_in=rhminh_arr(:,k))
 ...
 ! Sungsu added below diagnostic output
   ...
   call outfld( 'A_DET',              a_det_star,  pcols, lchnk )

 ! Sungsu added above diagnostic output
 ...
 ! Sungsu
   ...
   if( CAMstfrac ) then
       call astG_RHU(U0_in(:),p_in(:),qv0_in(:),landfrac(:),snowh(:),al0_st_nc_in(:),G0_nc_in(:),ncol,&
                     rhminl_in=rhminl_in(:),rhminl_adj_land_in=rhminl_adj_land_in(:),rhminh_in=rhminh_in(:))
   else
   ...
   call aist_vector(qv0_in(:),T0_in(:),p_in(:),qi0_in(:),landfrac(:),snowh(:),ai0_st_nc_in(:),ncol,&
                    rhmini_in=rhmini_in(:),rhminl_in=rhminl_in(:),&
                    rhminl_adj_land_in=rhminl_adj_land_in(:),rhminh_in=rhminh_in(:))
...
11 continue
 ...
 ! Sungsu
   real(r8), optional, intent(in)  :: rhminl                ! Critical relative humidity for low-level  liquid stratus
   real(r8), optional, intent(in)  :: rhminl_adj_land       ! Adjustment drop of rhminl over the land
   real(r8), optional, intent(in)  :: rhminh                ! Critical relative humidity for high-level liquid stratus
 ! Sungsu 
 ...
 ! Sungsu 
   ...
   subroutine aist_single( qv, T, p, qi, landfrac, snowh, aist, &
                           rhmini, rhminl, rhminl_adj_land, rhminh )
 ...
 ! Sungsu
   real(r8), optional, intent(in)  :: rhmini          ! Critical relative humidity for               ice stratus
   real(r8), optional, intent(in)  :: rhminl          ! Critical relative humidity for low-level  liquid stratus
   real(r8), optional, intent(in)  :: rhminl_adj_land ! Adjustment drop of rhminl over the land
   real(r8), optional, intent(in)  :: rhminh          ! Critical relative humidity for high-level liquid stratus
 ! Sungsu 
