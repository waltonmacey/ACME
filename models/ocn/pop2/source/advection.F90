...
!-----------------------------------------------------------------------
   ...
   if (mix_pass /= 1 ) then
      ...
      do n=1,nt
         ...
         ! Valid for both partial bottom cells or not and for any sfc_layer_type.
         do j=jb,je
         do i=ib,ie
           if (k == 1) then
             WORK(i,j) = WTKB(i,j)*(TRCR(i,j,k+1,n) - TRCR(i,j,k,  n))
           elseif (k < KMT(i,j,bid)) then
             WORK(i,j) = WTKB(i,j)*(TRCR(i,j,k+1,n) - TRCR(i,j,k,  n))&
                        +WTK(i,j) *(TRCR(i,j,k,n)   - TRCR(i,j,k-1,n))
           else
             WORK(i,j) = WTK(i,j) *(TRCR(i,j,k,n)   - TRCR(i,j,k-1,n))
           endif
         enddo
         enddo
         WORK = p5*WORK/DZT(:,:,k,bid)
