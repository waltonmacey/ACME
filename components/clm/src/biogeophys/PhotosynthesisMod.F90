         ...
         if ( .not. nu_com_leaf_physiology) then
            ...
            if ( CNAllocate_Carbon_only() .or. cnallocate_carbonphosphorus_only()) then
               ...
               if (total_lai > 0.0_r8 .and. sum_nscaler > 0.0_r8) then
                  ...
                  lnc(p) = min(max(lnc(p),0.0_r8),3.0_r8) ! based on doi: 10.1002/ece3.1173
               else                                                                    
                  lnc(p) = 0.0_r8                                                      
               end if
               ...
               if (pft%active(p) .and. (pft%itype(p) .ne. noveg)) then
                  ...
                  if (total_lai > 0.0_r8 .and. sum_nscaler > 0.0_r8) then
                     ...
                     lnc(p) = min(max(lnc(p),0.0_r8),3.0_r8) ! based on doi: 10.1002/ece3.1173
                     lpc(p) = min(max(lpc(p),0.0_r8),0.5_r8) ! based on doi: 10.1002/ece3.1173
                  else
                     lnc(p) = 0.0_r8
                     lpc(p) = 0.0_r8
                  end if
                  ...
                  if (lnc(p) >= 0.1_r8 .and. lnc(p) <=3.0_r8 .and. lpc(p) >= 0.05_r8 .and. lpc(p) <= 0.5_r8) then
                     ...
                     vcmax25top = 10.0_r8 * dayl_factor(p)
                     jmax25top  = 10.0_r8 * dayl_factor(p)
                  else
                     vcmax25top = 150.0_r8 * dayl_factor(p)
                     jmax25top  = 250.0_r8 * dayl_factor(p)
                  end if
