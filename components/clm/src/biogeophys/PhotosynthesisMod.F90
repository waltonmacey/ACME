  ...
  subroutine Photosynthesis ( bounds, fn, filterp, &
    ...
    associate(                                                       & 
      ...
      do f = 1, fn
         ...
         fnr   = veg_vp%fnr(pft_pp%itype(p))   !7.16_r8
         act25 = veg_vp%act25(pft_pp%itype(p)) !3.6_r8   !umol/mgRubisco/min
         ...
         kcha    = veg_vp%kcha(pft_pp%itype(p)) !79430._r8
         koha    = veg_vp%koha(pft_pp%itype(p)) !36380._r8
         cpha    = veg_vp%cpha(pft_pp%itype(p)) !37830._r8
         vcmaxha = veg_vp%vcmaxha(pft_pp%itype(p)) !72000._r8
         jmaxha  = veg_vp%jmaxha(pft_pp%itype(p)) !50000._r8
         tpuha   = veg_vp%tpuha(pft_pp%itype(p))  !72000._r8
         lmrha   = veg_vp%lmrha(pft_pp%itype(p))  !46390._r8

         ! High temperature deactivation, from:
         ! Leuning (2002) Plant, Cell and Environment 25:1205-1210
         ! The factor "c" scales the deactivation to a value of 1.0 at 25C

         vcmaxhd = veg_vp%vcmaxhd(pft_pp%itype(p)) !200000._r8
         jmaxhd  = veg_vp%jmaxhd(pft_pp%itype(p))  !200000._r8
         tpuhd   = veg_vp%tpuhd(pft_pp%itype(p))   !200000._r8
         lmrhd   = veg_vp%lmrhd(pft_pp%itype(p))   !150650._r8
         lmrse   = veg_vp%lmrse(pft_pp%itype(p))   !490._r8
         ...
         if (c3flag(p)) then
            qe(p)       = veg_vp%qe(pft_pp%itype(p))       !0._r8
            theta_cj(p) = veg_vp%theta_cj(pft_pp%itype(p)) !0.98_r8
            bbbopt(p)   = veg_vp%bbbopt(pft_pp%itype(p))   !10000._r8
            mbbopt(p)   = veg_vp%mbbopt(pft_pp%itype(p))   !9._r8
         else
            qe(p)       = veg_vp%qe(pft_pp%itype(p))       !0.05_r8
            theta_cj(p) = veg_vp%theta_cj(pft_pp%itype(p)) !0.80_r8
            bbbopt(p)   = veg_vp%bbbopt(pft_pp%itype(p))   !40000._r8
            mbbopt(p)   = veg_vp%mbbopt(pft_pp%itype(p))   !4._r8
         end if
