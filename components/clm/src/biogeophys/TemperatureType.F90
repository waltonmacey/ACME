  ...
  subroutine UpdateAccVars (this, EDbio_vars, bounds)
    ...
    if ( use_ed ) then
       ...
       do p = bounds%begp,bounds%endp
          ...
          if( grc_pp%latdeg(g) >= 0._r8.and.month >= 7 ) then !do not accumulate in latter half of year.
             rbufslp(p) = -99999._r8
          endif

          if( grc_pp%latdeg(g) < 0._r8.and.month < 6 ) then !do not accumulate in earlier half of year.
             rbufslp(p) = -99999._r8
          endif
