...
contains
       ...
       do while (associated(currentCohort)) 
          ...
          do z = 1,min(currentCohort%NV,nlevcan_ed-1)     
             if(currentCohort%year_net_uptake(z) /= 999._r8)then !there was activity this year in this leaf layer. 
                ...
                !decidous costs. 
                if(veg_pp%season_decid(currentCohort%pft) == 1.or.veg_pp%stress_decid(currentCohort%pft) == 1)then 
                   currentCohort%leaf_cost =  1._r8/(veg_pp%slatop(currentCohort%pft)*1000_r8)
                   currentCohort%leaf_cost = currentCohort%leaf_cost + 1.0_r8/(veg_pp%slatop(currentCohort%pft)*1000_r8) * &
                        veg_pp%froot_leaf(currentCohort%pft) / EDecophyscon%root_long(currentCohort%pft)
                   currentCohort%leaf_cost = currentCohort%leaf_cost * (ED_val_grperc+1._r8)
                else !evergreen costs
                   currentCohort%leaf_cost = 1.0_r8/(veg_pp%slatop(currentCohort%pft)* &
                        veg_pp%leaf_long(currentCohort%pft)*1000_r8) !convert from sla in m2g-1 to m2kg-1 
                   currentCohort%leaf_cost = currentCohort%leaf_cost + 1.0_r8/(veg_pp%slatop(currentCohort%pft)*1000_r8) * &
                        veg_pp%froot_leaf(currentCohort%pft) / EDecophyscon%root_long(currentCohort%pft)
                   currentCohort%leaf_cost = currentCohort%leaf_cost * (ED_val_grperc+1._r8)
                endif
                if(currentCohort%year_net_uptake(z) < currentCohort%leaf_cost)then
                   if(currentCohort%canopy_trim > trim_limit)then
                      ...
                      if(currentCohort%hite > EDecophyscon%hgt_min(currentCohort%pft))then
                         ...
                         if(veg_pp%evergreen(currentCohort%pft) /= 1)then
  ...
  subroutine phenology( g, temperature_vars, waterstate_vars)
    ...
    if(GDD0(currentSite%oldest_patch%clm_pno) > gdd_threshold.and.currentSite%status == 1)then
       if(timesinceleafoff > 0.9*(365-365/(1.0_r8/veg_pp%leaf_long(7))))then  
    ...
    if(timesinceleafon > 365/(1.0_r8/veg_pp%leaf_long(7)))then !!fudge - this  shouldn't be hard  wired. 
       if(currentSite%status == 2)then
          currentSite%status = 1        !alter status of site to 'leaves on'
          currentSite%leafoffdate = t   !record leaf off date   
          currentSite%gdd = 0._r8           !zero the GDD counter
   ...
   !we still haven't done budburst by end of window
    ...
    if(currentSite%dstatus == 2.and.t >= 10)then  !D*
       ...
       if(timesincedleafon > 365.0*veg_pp%leaf_long(7))then 
  ...
  subroutine phenology_leafonoff( g )
    ...
    do while(associated(currentPatch))    
       ...
       do while(associated(currentCohort))        
          ...
          !COLD LEAF ON
          if(veg_pp%season_decid(currentCohort%pft) == 1)then
          ...
          !DROUGHT LEAF ON
          if(veg_pp%stress_decid(currentCohort%pft) == 1)then
  ...
  subroutine Growth_Derivatives( currentCohort )
   ...
   ! calculate target size of living biomass compartment for a given dbh.   
    target_balive = Bleaf(currentCohort) * (1.0_r8 + veg_pp%froot_leaf(currentCohort%pft) + &
         EDecophyscon%sapwood_ratio(currentCohort%pft)*h)
    !target balive without leaves. 
    if(currentCohort%status_coh == 1)then 
       target_balive = Bleaf(currentCohort) * (veg_pp%froot_leaf(currentCohort%pft) + &
            EDecophyscon%sapwood_ratio(currentCohort%pft) * h)
    endif
    ...
    ! Maintenance demands     
    if(veg_pp%evergreen(currentCohort%pft) == 1)then !grass and EBT
       currentCohort%leaf_md = currentCohort%bl / veg_pp%leaf_long(currentCohort%pft)
    ...
    if(veg_pp%stress_decid(currentCohort%pft) /= 1.and.veg_pp%season_decid(currentCohort%pft) /= 1.and. &
         veg_pp%evergreen(currentCohort%pft) /= 1)then
       write(iulog,*) 'problem with phenology definitions',currentCohort%pft,veg_pp%stress_decid(currentCohort%pft), &
            veg_pp%season_decid(currentCohort%pft),veg_pp%evergreen(currentCohort%pft)
    endif
    ...
    if((currentCohort%balive >= target_balive).AND.(currentCohort%carbon_balance >  0._r8))then 
       ...
       if(currentCohort%dbh <= EDecophyscon%max_dbh(currentCohort%pft))then ! cap on leaf biomass
          ...
          dbrdbd = veg_pp%froot_leaf(currentCohort%pft) * dbldbd
  ...
  subroutine recruitment( t, currentPatch )
    ...
    do ft = 1,numpft_ed
       ...
       dc%dbh         = Dbh(dc)
       dc%bdead       = Bdead(dc)
       dc%balive      = Bleaf(dc)*(1.0_r8 + veg_pp%froot_leaf(ft) + EDecophyscon%sapwood_ratio(ft)*dc%hite)
       dc%bstore      = EDecophyscon%cushion(ft)*(dc%balive/ (1.0_r8 + veg_pp%froot_leaf(ft) &
       ...
       dc%laimemory = (1.0_r8/(1.0_r8 + veg_pp%froot_leaf(ft) + &
