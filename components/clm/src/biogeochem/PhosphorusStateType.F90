...
contains
  ...
  subroutine InitCold(this, bounds, &
    ...
    do p = bounds%begp,bounds%endp
       ...
       this%actual_leafcp(p)       = veg_pp%leafcp(pft_pp%itype(p))
       this%actual_frootcp(p)      = veg_pp%frootcp(pft_pp%itype(p))
       this%actual_livewdcp(p)     = veg_pp%livewdcp(pft_pp%itype(p))
       this%actual_deadwdcp(p)     = veg_pp%deadwdcp(pft_pp%itype(p))
       this%actual_graincp(p)      = veg_pp%graincp(pft_pp%itype(p))
