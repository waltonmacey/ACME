...
contains
  ...
  subroutine InitCold(this, bounds, &
    ...
    do p = bounds%begp,bounds%endp
       ...
       this%actual_leafcp(p)       = veg_vp%leafcp(veg_pp%itype(p))
       this%actual_frootcp(p)      = veg_vp%frootcp(veg_pp%itype(p))
       this%actual_livewdcp(p)     = veg_vp%livewdcp(veg_pp%itype(p))
       this%actual_deadwdcp(p)     = veg_vp%deadwdcp(veg_pp%itype(p))
       this%actual_graincp(p)      = veg_vp%graincp(veg_pp%itype(p))
