...
!    !-------------------------------------------------------------------
      ...
      do fp=1,num_soilp
         ...
         if ( nu_com .eq. 'RD') then
             ...
             do j = 1 , nlevdecomp
                 W_lim_factor(p) = W_lim_factor(p)  + w_scalar(c,j) 
             end do
             W_lim_factor(p) = W_lim_factor(p) / nlevdecomp
             ...
             retransn_to_npool(p) = max(0.0, retransn(p)/dt)
             retransp_to_ppool(p) = max(0.0, retransp(p)/dt)
