module CNStateType
  ...
  type, public :: cnstate_type
     ...
     real(r8) , pointer :: f1_patch                    (:)     ! patch allocation parameter
     real(r8) , pointer :: f2_patch                    (:)     ! patch allocation parameter
     real(r8) , pointer :: f3_patch                    (:)     ! patch allocation parameter
     real(r8) , pointer :: aroot_patch                 (:)     ! patch allocation parameter for crops
     real(r8) , pointer :: arepr_patch                 (:)     ! patch allocation parameter for crops
     real(r8) , pointer :: nlc_c_patch                 (:)     ! patch C based stoichiometry balance new leaf carbon
     real(r8) , pointer :: nlc_n_patch                 (:)     ! patch N based stoichiometry balance new leaf carbon
     real(r8) , pointer :: nlc_p_patch                 (:)     ! patch P based stoichiometry balance new leaf carbon
...
contains
  ...
  subroutine InitAllocate(this, bounds)
    ...
    allocate(this%f1_patch                    (begp:endp)) ; this%f1_patch(:) = nan
    allocate(this%f2_patch                    (begp:endp)) ; this%f2_patch(:) = nan
    allocate(this%f3_patch                    (begp:endp)) ; this%f3_patch(:) = nan
    allocate(this%aroot_patch                 (begp:endp)) ; this%aroot_patch(:) = nan
    allocate(this%arepr_patch                 (begp:endp)) ; this%arepr_patch(:) = nan

    allocate(this%nlc_c_patch                 (begp:endp)) ; this%nlc_c_patch(:) = nan
    allocate(this%nlc_n_patch                 (begp:endp)) ; this%nlc_n_patch(:) = nan
    allocate(this%nlc_p_patch                 (begp:endp)) ; this%nlc_p_patch(:) = nan
