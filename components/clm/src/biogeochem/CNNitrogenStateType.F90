...
contains
  ...
  subroutine InitCold(this, bounds, &
    ...
    do p = bounds%begp,bounds%endp
       ...
       this%actual_leafcn(p)       = veg_vp%leafcn(pft_pp%itype(p))
       this%actual_frootcn(p)      = veg_vp%frootcn(pft_pp%itype(p))
       this%actual_livewdcn(p)     = veg_vp%livewdcn(pft_pp%itype(p))
       this%actual_deadwdcn(p)     = veg_vp%deadwdcn(pft_pp%itype(p))
       this%actual_graincn(p)      = veg_vp%graincn(pft_pp%itype(p))
