...
contains
  ...
  subroutine InitCold(this, bounds, &
    ...
    do p = bounds%begp,bounds%endp
       ...
       this%actual_leafcn(p)       = ecophyscon%leafcn(pft_pp%itype(p))
       this%actual_frootcn(p)      = ecophyscon%frootcn(pft_pp%itype(p))
       this%actual_livewdcn(p)     = ecophyscon%livewdcn(pft_pp%itype(p))
       this%actual_deadwdcn(p)     = ecophyscon%deadwdcn(pft_pp%itype(p))
       this%actual_graincn(p)      = ecophyscon%graincn(pft_pp%itype(p))
