...
contains
  ...
  subroutine InitCold(this, bounds, &
    ...
    do p = bounds%begp,bounds%endp
       ...
       this%actual_leafcn(p)       = veg_pp%leafcn(pft_pp%itype(p))
       this%actual_frootcn(p)      = veg_pp%frootcn(pft_pp%itype(p))
       this%actual_livewdcn(p)     = veg_pp%livewdcn(pft_pp%itype(p))
       this%actual_deadwdcn(p)     = veg_pp%deadwdcn(pft_pp%itype(p))
       this%actual_graincn(p)      = veg_pp%graincn(pft_pp%itype(p))
