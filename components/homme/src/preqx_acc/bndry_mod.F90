...
module bndry_mod
  ...
  public :: bndry_exchangeV_simple_minimize_pcie

contains

  subroutine bndry_exchangeV_simple_minimize_pcie(hybrid,buffer)
    ...
    integer                   :: dest,length,tag,icycle,ierr,iptr,source,nlyr,nSendCycles,nRecvCycles,errorcode,errorlen,i,ithr
    ...
    if(hybrid%ithr == 0) then
      ...
      call t_startf('copy on-dev')
      !Copy internal data to receive buffer
      do ithr = 1 , hybrid%NThreads
        iptr   = buffer%moveptr(ithr)
        length = buffer%moveLength(ithr)
        if(length>0) call copy_ondev_async(buffer%receive(iptr),buffer%buf(iptr),length,ithr)
      enddo
      !$acc wait
      call t_stopf('copy on-dev')

      call MPI_Waitall(nRecvCycles,Rrequest,status,ierr)

      call t_startf('pci-e to device')
      do icycle = 1 , nRecvCycles
        pCycle => pSchedule%RecvCycle(icycle)
        iptr   =  pCycle%ptrP
        call update_device_async(buffer%receive(1+nlyr*(iptr-1)),nlyr*pCycle%lengthP,icycle)
      enddo
      !$acc wait
      call t_stopf('pci-e to device')

      call MPI_Waitall(nSendCycles,Srequest,status,ierr)

    endif  ! if (hybrid%ithr == 0)
    !$OMP BARRIER
  end subroutine bndry_exchangeV_simple_minimize_pcie
