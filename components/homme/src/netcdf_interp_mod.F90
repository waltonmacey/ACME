...
module netcdf_interp_mod
  ...
  use dimensions_mod,   only: nelemd, np, ne, nc, nlev
  ...
  !_____________________________________________________________________________
  subroutine write_scalar_field_1d(elem, var, ios, n0, hybrid)
    ...
    do ie=1,nelemd                                                      ! loop over elem
      ...
      element_data = get_scalar_field(elem(ie), var%short_name, n0, n_vert)! get scalar field by name
  ...
  subroutine write_vector_field(elem, var, ios, n0, hybrid)
  ...
  !_____________________________________________________________________________
  subroutine netcdf_interp_write(elem, tl, hybrid)
    ...
    do ios=1,max_output_streams                                         ! loop over all output streams
      ...
      if(( output_start_time(ios) <= tl%nstep)  .and. &
        ...
        do i=1,n_vars                                                   ! loop over all possible fields
          call write_scalar_field_1d(elem, nc_vars(i), ios, n0, hybrid)    ! write if scalar field and is-selected
        enddo

        ! write interpolated vector fields

        do i=1,n_vfields                                                ! loop over all vector fields
          call write_vector_field(elem,nc_vfields(i), ios, n0, hybrid)  ! write if at least one component selected
        enddo
