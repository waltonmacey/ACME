...
module namelist_mod
  ...
  !-----------------
  use thread_mod, only : nthreads, nthreads_accel, omp_set_num_threads, omp_get_max_threads, vert_num_threads, hthreads, vthreads
  !-----------------
...
! ^ ifndef CAM
    ...
    call MPI_bcast(hthreads  ,1,MPIinteger_t,par%root,par%comm,ierr)
...
! ^ ifndef CAM
    ...
    if (par%masterproc) then
       ...
       write(iulog,*)"readnl: nthreads      = ",nthreads
       write(iulog,*)"readnl: hthreads      = ",hthreads
