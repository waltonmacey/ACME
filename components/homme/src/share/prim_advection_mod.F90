...
!
  ...
  use dimensions_mod, only     : nlev, nlevp, np, qsize, nep
  use physical_constants, only : rgas, Rwater_vapor, kappa, g, rearth, rrearth, cp
  use derivative_mod, only     : gradient, vorticity, gradient_wk, derivative_t, divergence, gradient_sphere, divergence_sphere
  ...
  use edge_mod, only           : edgevpack, edgerotate, edgevunpack, initedgebuffer, initedgesbuffer, edgevunpackmin, initghostbuffer3D
  use edgetype_mod, only       : EdgeDescriptor_t, EdgeBuffer_t, ghostbuffer3D_t
  use hybrid_mod, only         : hybrid_t
  use bndry_mod, only          : bndry_exchangev
  use viscosity_mod, only      : biharmonic_wk_scalar, biharmonic_wk_scalar_minmax, neighbor_minmax, neighbor_minmax_start, neighbor_minmax_finish
...
contains
  ...
  subroutine  Prim_Advec_Tracers_remap_ALE( elem , deriv , hybrid , dt , tl , nets , nete )
    ...
    do ie=nets,nete
      ...
      do k=1,nlev
        do q=1,qsize
          ! note: pack so that tracers per level are contiguous so we can unpack into array neigh_q()
  ...
  subroutine Cobra_SLBQP(Que, Que_t, rho, minq, maxq, mass, hybrid, nets, nete) 
    ...
    integer             , intent(in   ) :: nets
    integer             , intent(in   ) :: nete
    real(kind=real_kind), intent(inout) :: Que  (np*np,nlev,qsize,nets:nete)
  ...
  subroutine ALE_RKdss(elem, nets, nete, hy, deriv, dt, tl)
    ...
    do ie=nets,nete
      ...
      do k=1,nlev
        ...
        elem(ie)%derived%vstar(:,:,:,k) = (elem(ie)%state%v(:,:,:,k,np1) + elem(ie)%derived%vstar(:,:,:,k))/2 - dt*vtmp(:,:,:)/2
  ...
  subroutine ALE_departure_from_gll(acart, vstar, elem, dt)
    ...
    implicit none
    type(cartesian3D_t)   ,intent(inout) :: acart(np,np)
  ...
  subroutine ALE_elems_with_dep_points (elem_indexes, dep_points, num_neighbors, ngh_corners)
    ...
    integer              , intent(inout) :: elem_indexes(np,np)
...
#         endif
          do q = 1 , qsize
            do k = 1 , nlev    !  Loop inversion (AAM)
              ! note: biharmonic_wk() output has mass matrix already applied. Un-apply since we apply again below:
              qtens_biharmonic(:,:,k,q,ie) = -rhs_viss*dt*nu_q*dp0(k)*Qtens_biharmonic(:,:,k,q,ie) / elem(ie)%spheremp(:,:)
            enddo
          enddo
        enddo
#       endif
...
#     endif
      do q = 1 , qsize
        ...
        if ( limiter_option == 8) then
          ...
          call limiter_optim_iter_full( Qtens(:,:,:) , elem(ie)%spheremp(:,:) , qmin(:,q,ie) , qmax(:,q,ie) , dpdissk )
        endif
...
#   endif
  ...
  subroutine advance_hypervis_scalar( edgeAdv , elem , hvcoord , hybrid , deriv , nt , nt_qdp , nets , nete , dt2 )
    ...
    ! local
    real (kind=real_kind) :: Qtens    (np,np,nlev,qsize,nets:nete)
...
#       endif
        do k = 1 , nlev
          ...
          if (nu_p>0) then
            do q = 1 , qsize
              Qtens(:,:,k,q,ie) = elem(ie)%derived%dpdiss_ave(:,:,k)*elem(ie)%state%Qdp(:,:,k,q,nt_qdp) / dp(:,:,k)
            enddo
          else
            do q = 1 , qsize
  ...
  do ie=nets,nete
    ...
    if (rsplit==0) then
      ...
      do k=1,nlev
        ...
        dp_star(:,:,k) = dp(:,:,k) + dt*(elem(ie)%derived%eta_dot_dpdn(:,:,k+1) - elem(ie)%derived%eta_dot_dpdn(:,:,k))
      enddo
...
#      endif
       ...
       if ( .not. se_prescribed_wind_2d ) elem(ie)%state%v(:,:,1,:,np1)=ttmp(:,:,:,1)/dp
       if ( .not. se_prescribed_wind_2d ) elem(ie)%state%v(:,:,2,:,np1)=ttmp(:,:,:,2)/dp
#      ifdef REMAP_TE
       ! back out T from TE
