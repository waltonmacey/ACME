...
module prim_advection_mod
  ...
  subroutine advect_tracers(elem, hybrid_, nets_, nete_, hvcoord_, dt, tl)
    ...
    hybrid=hybrid_; nets=nets_; nete=nete_; hv=hvcoord_
  ...
  subroutine euler_step(elem,n0,n1,n2,dt,tl)
    ...
    do ie=nets,nete
      ...
      do iq=1,qsize
        ...
        do k=1,nlev; grad_q(:,:,:,k) = gradient_sphere( q(:,:,k,iq,n1,ie),deriv(hybrid%ithr), elem(ie)%Dinv); enddo
